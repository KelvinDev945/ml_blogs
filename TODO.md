...
- [ ] **【当前任务】为358篇文章添加极详细数学推导（91/358篇已完成，25.4%）**
  ...
  - **第九批（最近37篇blog，2026-01-05进行中）** 🔄：
    ...
    * **子批次C：优化理论与训练技巧（10篇，2026-01-08进行中）** 🔄
      - [x] 让炼丹更科学一些（二）：将结论推广到无界域（1266行，64公式）✅
      - [x] 让炼丹更科学一些（三）：SGD的终点损失收敛（944行，41公式）✅
      - [x] 让炼丹更科学一些（四）：新恒等式，新学习率（864行，40公式）✅
      - [ ] 从动力学角度看优化算法（五）：为什么学习率不宜过小（278行→需扩充）⏳
      - [ ] 从动力学角度看优化算法（六）：为什么SimSiam不退化（233行→需扩充）⏳
      - [ ] 从动力学角度看优化算法（七）：SGD ≈ SVM（201行→需扩充）⏳
      - [ ] 为什么梯度裁剪能加速训练过程：一个简明的分析（待扩充）⏳
      - [ ] 隐藏在动量中的梯度累积：少更新几步效果反而更好（待扩充）⏳
      - [ ] 泛化性乱弹：从随机噪声、梯度惩罚到虚拟对抗训练（待扩充）⏳
      - [ ] 滑动平均视角下的权重衰减和学习率（待扩充）⏳
    * **子批次D：Attention/Transformer架构（13篇）** ⏳
    * **子批次E：NLP应用与其他（5篇）** ⏳
  - **📊 进度总结**：
    * ✅ 已完成8批+第9批部分共94篇文章（26.3%）
    * 📝 剩余264篇文章待完成（73.7%）
    ...
    * 🔄 第九批子批次C优化理论3/10篇已完成（3074行，145公式）
- [x] 优化首页搜索功能（添加标签筛选）
- [x] 添加文章分类功能
