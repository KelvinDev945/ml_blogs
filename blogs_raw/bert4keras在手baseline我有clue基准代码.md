...
**定义**: 自回归语言模型通过链式法则分解联合概率
$$P(x_1, x_2, \ldots, x_T) = \prod_{t=1}^T P(x_t | x_1, \ldots, x_{t-1}) \tag{1}$$

**数学直觉**: 将序列生成问题分解为逐步预测问题，每一步只依赖历史信息。

**对数似然目标函数**:
$$\mathcal{L}_{\text{AR}} = -\sum_{t=1}^T \log P_\theta(x_t | x_{<t}) \tag{2}$$

其中$x_{<t} = (x_1, \ldots, x_{t-1})$表示$t$时刻之前的所有token。

**推导注释**: 最大似然估计等价于最小化负对数似然，这是语言模型训练的基础。

#### 1.2 BERT的掩码语言模型(MLM)

**核心思想**: 不同于自回归模型，BERT采用**双向**编码，通过掩码机制进行预训练。

**掩码策略**: 给定输入序列$\mathbf{x} = (x_1, \ldots, x_T)$，随机选择15%的位置集合$\mathcal{M}$进行掩码：
- 80%的概率替换为[MASK]
- 10%的概率替换为随机token
- 10%的概率保持不变

**MLM目标函数**:
$$\mathcal{L}_{\text{MLM}} = -\mathbb{E}_{\mathbf{x} \sim \mathcal{D}} \sum_{i \in \mathcal{M}} \log P_\theta(x_i | \mathbf{x}_{\setminus \mathcal{M}}) \tag{3}$$

其中$\mathbf{x}_{\setminus \mathcal{M}}$表示被掩码的输入序列。
...
**信息论分析**: 设$H(\cdot)$为熵，则双向上下文提供更多信息：
$$H(X_t | X_{<t}) \geq H(X_t | X_{<t}, X_{>t}) \tag{6}$$

不等式表明双向条件下的不确定性更低，预测更准确。

#### 1.3 下一句预测(NSP)

**任务定义**: 判断两个句子$A$和$B$是否在原文中连续出现。

**数学形式化**: 设$(s_A, s_B)$为句子对，$y \in \{0, 1\}$为标签：
$$P(y = 1 | s_A, s_B) = \sigma(\mathbf{w}^\top \mathbf{h}_{[CLS]} + b) \tag{7}$$

其中$\mathbf{h}_{[CLS]}$是[CLS] token的隐藏表示，$\sigma$是sigmoid函数。

**联合训练目标**:
$$\mathcal{L}_{\text{BERT}} = \mathcal{L}_{\text{MLM}} + \mathcal{L}_{\text{NSP}} \tag{8}$$

**注**: 后续研究(如RoBERTa)发现NSP任务可能不是必需的，但在原始BERT中起到辅助作用。

### 二、Transformer架构的数学原理

#### 2.1 自注意力机制(Self-Attention)

**基本形式**: 给定输入序列的表示$\mathbf{X} \in \mathbb{R}^{T \times d}$，计算：
...
假设$\mathbf{Q}$和$\mathbf{K}$的元素是独立同分布的随机变量，均值为0，方差为1。

设$q_i, k_j$是$\mathbf{Q}$和$\mathbf{K}$中的元素，则点积：
$$q \cdot k = \sum_{i=1}^{d_k} q_i k_i \tag{11}$$
...
**结论**: 点积的标准差为$\sqrt{d_k}$。为了保持方差为1，需要除以$\sqrt{d_k}$。

**softmax稳定性**: 当输入过大时，softmax会产生极端的概率分布：
$$\lim_{x \to \infty} \text{softmax}([x, 0, 0, \ldots]) = [1, 0, 0, \ldots] \tag{14}$$
...
- 总参数量与单头相同: $4d_{\text{model}}^2$

**理论优势**: 多头机制允许模型在不同的表示子空间中捕获不同的依赖关系。

**信息论视角**: 设$I(\cdot; \cdot)$为互信息，则：
$$I(X; Y_{\text{multi-head}}) \geq I(X; Y_{\text{single-head}}) \tag{17}$$

多头提供了更丰富的特征表示。

#### 2.3 位置编码(Positional Encoding)

**必要性**: 自注意力是置换不变的(permutation-invariant)，需要位置信息。
...
**理论性质**:
1. **线性关系**: 对于任意固定的偏移$k$，$PE_{pos+k}$可以表示为$PE_{pos}$的线性函数
2. **周期性**: 不同频率的正弦函数编码不同尺度的位置信息

**相对位置表示**: 设$\omega_i = 1/10000^{2i/d}$，则：
$$\begin{pmatrix} \sin(\omega_i(pos+k)) \\ \cos(\omega_i(pos+k)) \end{pmatrix} = \begin{pmatrix} \cos(\omega_i k) & \sin(\omega_i k) \\ -\sin(\omega_i k) & \cos(\omega_i k) \end{pmatrix} \begin{pmatrix} \sin(\omega_i pos) \\ \cos(\omega_i pos) \end{pmatrix} \tag{20}$$

这保证了相对位置关系可以通过线性变换学习。

### 三、BERT分类任务的数学建模

#### 3.1 文本分类任务

**输入表示**: 输入序列$\mathbf{x} = ([CLS], x_1, \ldots, x_T, [SEP])$

**BERT编码**: 经过BERT编码器后得到隐藏状态序列：
$$\mathbf{H} = (\mathbf{h}_{[CLS]}, \mathbf{h}_1, \ldots, \mathbf{h}_T, \mathbf{h}_{[SEP]}) \in \mathbb{R}^{(T+2) \times d} \tag{21}$$
...
**交叉熵损失**:
$$\mathcal{L}_{\text{cls}} = -\sum_{c=1}^C y_c \log P(y = c | \mathbf{x}) \tag{24}$$

**为什么使用[CLS]**: [CLS]没有对应具体的输入token，因此可以聚合全局信息。

**理论分析**: 在自注意力机制下，[CLS]的表示为：
$$\mathbf{h}_{[CLS]} = \sum_{i=0}^{T+1} \alpha_i \mathbf{v}_i \tag{25}$$

其中$\alpha_i$是注意力权重，这是一个全局加权平均。

#### 3.2 文本匹配任务

**输入形式**: $\mathbf{x} = ([CLS], s_A, [SEP], s_B, [SEP])$

**句子对表示**: BERT通过位置ID区分两个句子：
$$\begin{aligned}
\mathbf{h}_i &= \text{BERT}(x_i, \text{seg}_i, \text{pos}_i) \tag{26}
\end{aligned}$$

其中$\text{seg}_i \in \{0, 1\}$是句子段ID。

**匹配分数计算**:
$$\text{score}(s_A, s_B) = \sigma(\mathbf{w}^\top \mathbf{h}_{[CLS]} + b) \tag{27}$$

**对比**: 传统双塔模型分别编码：
$$\text{score}(s_A, s_B) = \text{sim}(f(s_A), f(s_B)) \tag{28}$$

BERT的交互式编码能捕获更细粒度的语义关系。

**信息论优势**: 设$I_{\text{BERT}}$和$I_{\text{dual}}$分别为两种方法的互信息：
$$I(s_A; s_B | \text{joint encoding}) \geq I(s_A; s_B | \text{separate encoding}) \tag{29}$$

### 四、GlobalPointer的数学原理

#### 4.1 传统序列标注方法的局限性

**BIO标注**: 将span抽取转化为token级分类
- 问题: 无法有效处理嵌套实体
- 损失: $\mathcal{L} = -\sum_{t=1}^T \log P(y_t | \mathbf{x})$ (独立假设)

**CRF**: 建模标签序列的依赖
$$P(\mathbf{y} | \mathbf{x}) = \frac{\exp(\text{score}(\mathbf{x}, \mathbf{y}))}{\sum_{\mathbf{y}'} \exp(\text{score}(\mathbf{x}, \mathbf{y}'))} \tag{30}$$

其中：
$$\text{score}(\mathbf{x}, \mathbf{y}) = \sum_{t=1}^T \psi(y_{t-1}, y_t, \mathbf{x}, t) \tag{31}$$

**局限**: 仍然无法处理嵌套和不连续的span。

#### 4.2 GlobalPointer的核心思想

**直接建模span**: 将实体抽取视为span的分类问题。

**span表示**: 对于span $(i, j)$（从位置$i$到$j$），计算分数：
$$s(i, j) = \mathbf{q}_i^\top \mathbf{k}_j \tag{32}$$

其中$\mathbf{q}_i, \mathbf{k}_j$是位置$i$和$j$的查询和键表示。

**多头GlobalPointer**: 为每个实体类型$c$定义独立的注意力头：
$$s_c(i, j) = \mathbf{q}_{c,i}^\top \mathbf{k}_{c,j} \tag{33}$$
...
**位置相对性**: 旋转位置编码保证：
$$\mathbf{q}_{c,i}^\top \mathbf{k}_{c,j} \text{ 只依赖于 } j - i \tag{37}$$
...
#### 4.3 GlobalPointer的损失函数

**二分类形式**: 对每个可能的span $(i, j)$判断是否为实体：
$$P(y_{ij}^c = 1 | \mathbf{x}) = \sigma(s_c(i, j)) \tag{39}$$

**多标签损失**:
$$\mathcal{L}_{\text{GP}} = -\sum_{c=1}^C \sum_{i \leq j} \left[ y_{ij}^c \log \sigma(s_c(i,j)) + (1-y_{ij}^c) \log(1-\sigma(s_c(i,j))) \right] \tag{40}$$

**Circle Loss变体**: 为了更好地区分正负样本：
$$\mathcal{L}_{\text{circle}} = \log\left(1 + \sum_{(i,j) \in \mathcal{P}} \sum_{(k,l) \in \mathcal{N}} \exp(s_c(k,l) - s_c(i,j))\right) \tag{41}$$

其中$\mathcal{P}$是正样本集合，$\mathcal{N}$是负样本集合。

### 五、阅读理解任务的数学建模

#### 5.1 抽取式阅读理解

**任务定义**: 给定问题$q$和段落$p$，抽取答案span $(s, e)$。
...
**训练**: 最大化$P_{\text{start}}(s) \cdot P_{\text{end}}(e)$

**推理**: 选择$\arg\max_{i \leq j} [P_{\text{start}}(i) \cdot P_{\text{end}}(j)]$

#### 5.2 使用GlobalPointer的联合预测

**联合span分数**:
$$s(i, j) = \mathbf{q}_i^\top \mathbf{k}_j, \quad i \leq j \tag{44}$$

**归一化**:
$$P(s, e | q, p) = \frac{\exp(s(s, e))}{\sum_{i \leq j} \exp(s(i, j))} \tag{45}$$

**训练和推理一致**: 都是最大化$P(s, e | q, p)$。

**理论优势**:
1. **联合建模**: 考虑起始和结束位置的相互依赖
2. **位置感知**: RoPE编码保证对span长度的建模
3. **快速解码**: 只需一次前向传播
...
**输入**: 上下文$c$，问题$q$，候选答案$\{a_1, \ldots, a_K\}$
...
**方法**: 为每个候选构造输入：
$$\mathbf{x}_k = [CLS] + c + [SEP] + q + [SEP] + a_k + [SEP] \tag{46}$$

**匹配分数**:
$$s_k = f_\theta(\mathbf{x}_k) = \mathbf{w}^\top \mathbf{h}_{[CLS]}^{(k)} \tag{47}$$

**预测分布**:
$$P(a = a_k) = \frac{\exp(s_k)}{\sum_{k'=1}^K \exp(s_{k'})} \tag{48}$$
...
传统方法（先分别编码再交互）:
$$s_k = \text{sim}(f(c, q), f(a_k)) \tag{49}$$

BERT方法（联合编码）:
$$s_k = f([c; q; a_k]) \tag{50}$$

**信息论分析**: 联合编码能捕获三元组之间的高阶交互：
$$I(c, q; a_k | \text{joint}) \geq I(c, q; a_k | \text{separate}) \tag{51}$$

**复杂度分析**:
- 前向传播次数: $K$次（$K$是候选数）
- 总计算量: $O(K \cdot T^2 \cdot d)$，其中$T$是序列长度
...
- 候选成语: $\{i_1, \ldots, i_n\}$（$n > m$）

**目标**: 为每个空位选择不同的成语

#### 7.2 分解为子问题

**单空位建模**: 对于空位$\#_k$，用[unused0]标记，其他用[MASK]:
$$c_k = [CLS] + c(\#_k \to [unused0], \#_{j \neq k} \to [MASK]^4) + [SEP] \tag{52}$$

**候选拼接**:
$$\mathbf{x}_{k,i} = c_k + [SEP] + \text{idiom}_i + [SEP] \tag{53}$$

**匹配分数矩阵**: $\mathbf{S} \in \mathbb{R}^{m \times n}$
$$S_{ki} = f_\theta(\mathbf{x}_{k,i}) \tag{54}$$
...
**优化问题**:
$$\max_{\pi} \sum_{k=1}^m S_{k, \pi(k)} \tag{55}$$

其中$\pi: [m] \to [n]$是一个单射(注入)函数。

**等价最小化问题**:
$$\min_{\pi} \sum_{k=1}^m (-S_{k, \pi(k)}) = \min_{\pi} \sum_{k=1}^m C_{k, \pi(k)} \tag{56}$$
...
**理论解释**: 避免初始阶段的大梯度破坏预训练权重。

**梯度范数分析**: 在微调初期，梯度范数较大：
$$\|\nabla \mathcal{L}\|_2 \approx \mathcal{O}(1/\sqrt{\eta}) \tag{58}$$
...
**熵分析**:
$$H(\tilde{y}) = -(1-\epsilon)\log(1-\epsilon+\epsilon/K) - (K-1)\frac{\epsilon}{K}\log\frac{\epsilon}{K} > 0 \tag{61}$$
...
**源域**: $\mathcal{D}_s = \{(\mathbf{x}_i^s, y_i^s)\}$（预训练数据）
**目标域**: $\mathcal{D}_t = \{(\mathbf{x}_i^t, y_i^t)\}$（下游任务数据）

**假设**: $P_s(\mathbf{x}) \neq P_t(\mathbf{x})$ 但存在共享的语义表示空间

**表示学习**: 预训练学习映射$f_\theta: \mathcal{X} \to \mathcal{H}$

**微调**: 学习任务特定函数$g_\phi: \mathcal{H} \to \mathcal{Y}$

**联合优化**:
$$\min_{\theta, \phi} \mathbb{E}_{(\mathbf{x}, y) \sim \mathcal{D}_t} \mathcal{L}(g_\phi(f_\theta(\mathbf{x})), y) + \lambda \|\theta - \theta_0\|^2 \tag{62}$$

其中$\theta_0$是预训练参数，$\lambda$控制正则化强度。

#### 9.2 为什么预训练有效？

**PAC学习理论**: 设$\mathcal{H}$是假设空间，VC维为$d$。样本复杂度：
$$m = O\left(\frac{d + \log(1/\delta)}{\epsilon}\right) \tag{63}$$

**预训练的作用**: 缩小假设空间$\mathcal{H}$，降低VC维$d$，从而减少所需样本数。

**信息瓶颈理论**: 预训练学习压缩表示$\mathbf{h} = f(\mathbf{x})$，满足：
$$\min I(\mathbf{X}; \mathbf{H}) \quad \text{s.t.} \quad I(\mathbf{H}; Y) \geq I_{\min} \tag{64}$$

#### 9.3 微调的收敛性分析

**假设**: 损失函数$\mathcal{L}$是$L$-smooth和$\mu$-strongly convex

**定理**: 使用学习率$\eta < 1/L$，微调$T$步后：
$$\mathbb{E}[\|\theta_T - \theta^*\|^2] \leq (1 - \mu\eta)^T \|\theta_0 - \theta^*\|^2 + \frac{\eta \sigma^2}{\mu} \tag{65}$$

其中$\sigma^2$是梯度方差。

**结论**: 好的初始化$\theta_0$（接近最优$\theta^*$）加速收敛。
...
**Embedding层**:
$$P_{\text{emb}} = V \cdot d + T_{\max} \cdot d + 2 \cdot d = 21128 \times 768 + 512 \times 768 + 2 \times 768 \approx 16.7M \tag{66}$$
...
**总参数量**:
$$P_{\text{total}} = P_{\text{emb}} + L \times P_{\text{layer}} = 16.7M + 12 \times 7.08M \approx 102M \tag{70}$$

与官方的110M接近（差异来自bias和LayerNorm参数）。
...
- 总计: $O(T \cdot d^2 + T^2 \cdot d)$

**Feed-Forward**:
$$O(2 \times T \times d \times d_{\text{ffn}}) = O(2 \times 128 \times 768 \times 3072) \approx 604M \tag{71}$$

**单层总FLOPs**: 约680M

**12层总FLOPs**: 约8.2B (billion)
...
**单步时间**:
$$t_{\text{step}} = \frac{\text{batch\_size} \times \text{FLOPs}}{\text{GPU\_throughput}} = \frac{32 \times 8.2B}{125T} \approx 2.1 \text{ms} \tag{72}$$

**总训练时间** (仅前向):
$$T_{\text{train}} = 1M \times 2.1 \text{ms} \approx 35 \text{minutes} \tag{73}$$

考虑反向传播（约2倍前向）和通信开销，实际约2-3小时。
...
**理论**: 保持梯度在FP16有效范围$[2^{-24}, 2^{15}]$内。

#### 11.2 梯度累积

**目标**: 模拟大batch size$B_{\text{eff}}$，实际使用小batch size$B_{\text{actual}}$
...
**经验法则**: 对于Adam优化器：
$$\eta \in [10^{-5}, 5 \times 10^{-5}] \tag{77}$$

**理论依据**: 预训练权重已接近最优，需要小步调整

**自适应策略**: 使用学习率范围测试(LR Range Test)
$$\eta_{\text{optimal}} = \arg\max_\eta \left\{ \eta : \frac{d\mathcal{L}}{d\eta} < 0 \right\} \tag{78}$$

#### 12.2 Batch Size选择

**内存约束**:
$$\text{Memory} \approx P_{\text{model}} \times (4 + 12 \times B \times T) \text{ bytes} \tag{79}$$

其中因子4来自FP32参数，12来自激活值和梯度。

**临界Batch Size**: 超过此值收益递减
$$B_{\text{critical}} \approx \frac{\epsilon}{\text{noise\_scale}^2} \tag{80}$$
...
**Dropout**: 在注意力和FFN层应用
$$\mathbf{h}' = \text{Dropout}(\mathbf{h}, p) = \frac{1}{1-p} \mathbf{h} \odot \mathbf{m}, \quad \mathbf{m} \sim \text{Bernoulli}(1-p) \tag{81}$$

**Weight Decay**: L2正则化
$$\mathcal{L}_{\text{reg}} = \mathcal{L} + \frac{\lambda}{2} \sum_i \theta_i^2 \tag{82}$$

**推荐值**: $\lambda \in [0.01, 0.1]$

### 总结

本节详细推导了BERT及其在CLUE基准任务上的数学原理，包括：

1. **理论基础**: 从自回归模型到双向编码，从自注意力到多头机制
2. **任务建模**: 文本分类、匹配、阅读理解、多项选择、成语理解
3. **优化技巧**: 学习率调度、梯度裁剪、混合精度、梯度累积
4. **理论分析**: 信息论、PAC学习、收敛性分析
5. **工程实践**: 参数量计算、时间估算、超参数选择

这些推导揭示了BERT强大性能背后的数学原理，为实践提供了理论指导。
