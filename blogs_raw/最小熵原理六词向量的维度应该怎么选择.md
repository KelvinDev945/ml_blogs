...
tags: 最小熵原理, 熵, 词向量, 最小熵, 生成模型
status: pending
---

# 最小熵原理（六）：词向量的维度应该怎么选择？

**原文链接**: [https://spaces.ac.cn/archives/7695](https://spaces.ac.cn/archives/7695)
...
由于没有找到比较满意的现有结果，因此笔者从[最小熵原理](/tag/%E6%9C%80%E5%B0%8F%E7%86%B5/)角度分析了一下，得到了一个接近自己心中所想的答案。
...
我们的出发点是信息熵，我们知道，熵是不确定性的度量（参考[《“熵”不起：从熵、最大熵原理到最大熵模型（一）》](/archives/3534)），语言本身具有一定的不确定性，而我们在用向量编码词语时，编码结果应该要等于甚至小于这种不确定性，才能保证这种编码是有效的、能充分保留原来语言的信息。所以，我们要消除不确定性，也就是要最小熵。
...
现在$\theta$相当于$n$维空间中任意两个向量的夹角，我们在[《n维空间下两个随机向量的夹角分布》](/archives/7076)中就求出了它的分布为  
...
以及注意当$n$足够大的时候，被积函数只有在$\theta=0.904557$附近才会明显不等于0（或者将正态分布的极限视为[狄拉克函数](/archives/1870)的近似），从而有  
\begin{equation}\begin{aligned}  
...
_**转载到请包括本文地址：**<https://spaces.ac.cn/archives/7695>_
...
苏剑林. (Aug. 20, 2020). 《 最小熵原理（六）：词向量的维度应该怎么选择？ 》[Blog post]. Retrieved from <https://spaces.ac.cn/archives/7695>
...
year={2020},  
month={Aug},  
url={\url{https://spaces.ac.cn/archives/7695}},  
} 
