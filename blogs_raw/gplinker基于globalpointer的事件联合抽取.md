...
**原文链接**: [https://spaces.ac.cn/archives/8926](https://spaces.ac.cn/archives/8926)
...
大约两年前，笔者在百度的"[2020语言与智能技术竞赛](http://lic2020.cipsc.org.cn/)"中首次接触到了事件抽取任务，并在文章[《bert4keras在手，baseline我有：百度LIC2020》](/archives/7321)中分享了一个转化为BERT+CRF做NER的简单baseline。不过，当时的baseline更像是一个用来凑数的半成品，算不上一个完整的事件抽取模型。而这两年来，关系抽取的模型层见迭出，SOTA一个接一个，但事件抽取似乎没有多亮眼的设计。

最近笔者重新尝试了事件抽取任务，在之前的关系抽取模型[GPLinker](/archives/8888)的基础上，结合完全子图搜索，设计一个比较简单但相对完备的事件联合抽取模型，依然称之为GPLinker，在此请大家点评一番。
...
[![标准的事件抽取样本（图片来自百度DuEE的GitHub）](/usr/uploads/2022/02/2216331038.png)](/usr/uploads/2022/02/2216331038.png "点击查看原图")
...
为了统一有无触发词的场景，我们这里就触发词也当作事件的一个论元角色，这样有无触发词就是增减一个论元而已，所以主要还是在于论元识别和事件划分。对于论元识别，我们还是将(事件类型, 论元角色)组合成一个大类，然后转化为NER问题，但要注意的是，不同的实体是可能嵌套的，所以之前基于CRF的NER其实是不够用的，这里用能识别嵌套实体的[GlobalPointer](/archives/8373)来完成。
...
[![有向图上的完全子图示例](/usr/uploads/2022/02/3286545116.png)](/usr/uploads/2022/02/3286545116.png "点击查看原图")

有向图上的完全子图示例

那么如何构建这个无向图呢？我们沿用TPLinker的做法，如果两个论元实体有关系，那么它们的(首, 首)和(尾, 尾)都能匹配上，我们可以像[关系抽取的GPLinker](/archives/8888)一样，用GlobalPointer来预测它们的匹配关系。特别地，由于我们只需要构建一个无向图，所以我们可以mask掉下三角部分，所有的边都只用上三角部分描述。
...
最后，代码用的GlobalPointer，全部都是[Efficient GlobalPointer](/archives/8877)，笔者也比较过使用标准版的GlobalPointer，发现它前期收敛更快但最终效果会更差。这也再次肯定了Efficient GlobalPointer的有效性。
...
_**转载到请包括本文地址：**<https://spaces.ac.cn/archives/8926>_
...
苏剑林. (Feb. 21, 2022). 《GPLinker：基于GlobalPointer的事件联合抽取 》[Blog post]. Retrieved from <https://spaces.ac.cn/archives/8926>
...
year={2022},  
month={Feb},  
url={\url{https://spaces.ac.cn/archives/8926}},  
} 
