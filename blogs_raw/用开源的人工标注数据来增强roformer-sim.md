...
**原文链接**: [https://spaces.ac.cn/archives/8541](https://spaces.ac.cn/archives/8541)

**发布日期**: 

---

大家知道，从[SimBERT](/archives/7427)到[SimBERTv2（RoFormer-Sim）](/archives/8454)，我们算是为中文文本相似度任务建立了一个还算不错的基准模型。然而，SimBERT和RoFormer-Sim本质上都只是“弱监督”模型，跟“无监督”类似，我们不能指望纯弱监督的模型能达到完美符合人的认知效果。所以，为了进一步提升RoFormer-Sim的效果，我们尝试了使用开源的一些标注数据来辅助训练。本文就来介绍我们的探索过程。
...
> 1、**是非类型** ：这种是比较常见的类型，主要格式是“(句子1, 句子2, 是否相似)”，[这里](/archives/8321)收集到的ATEC、BQ、LCQMC、PAWSX都是这种类型；
> 
...
[![不同拼接特征的实验结果](/usr/uploads/2021/07/1558247210.png)](/usr/uploads/2021/07/1558247210.png "点击查看原图")
...
为此，我们通过上述方式训练好一个Sentence-BERT后，通过[《SimBERTv2来了！融合检索和生成的RoFormer-Sim模型》](/archives/8454)介绍的方案，把Sentence-BERT的检索效果蒸馏到RoFormer-Sim上去，从而在保留相似句生成的基础上提高检索模型的效果。此外，同尺寸模型之间的蒸馏往往还能提升一点效果，所以我们蒸馏后的RoFormer-Sim的检索效果，其实还比直接训练得到的Sentence-BERT效果要好些。
...
下面是[《无监督语义相似度哪家强？我们做了个比较全面的评测》](/archives/8321)中的几个任务的测试结果（测试集）：  
\begin{array}{c|ccccc}  
\hline  
...
_**转载到请包括本文地址：**<https://spaces.ac.cn/archives/8541>_
