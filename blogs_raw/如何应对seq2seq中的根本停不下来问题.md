...
tags: 语言模型, 文本生成, 解码, 生成模型, attention
status: pending
---

# 如何应对Seq2Seq中的“根本停不下来”问题？

**原文链接**: [https://spaces.ac.cn/archives/7500](https://spaces.ac.cn/archives/7500)
...
随机性解码算法，顾名思义，就是哪怕输入文本固定了，解码出来的输出文本也不是固定的，比如从训练语言模型进行随机采样就是这这种算法（参考[《现在可以用Keras玩中文GPT2了》](/archives/7292)）。对于Seq2Seq来说，我们很多时候希望得到确定性的结果，所以多数场景下我们都是用Beam Search。但是Beam Searc的生成结果可能会出现过于单一的现象（即类似“好的”、“不知道”、“谢谢”这类比较“安全”的回复），或者有时候我们希望增加输出的多样性（比如我们之前开源的做相似句生成的[SimBERT](/archives/7427)模型），这时候就需要随机性解码算法，它包含三种情况：原生随机解码、top-k随机解码、Nucleus随机解码。
...
_**转载到请包括本文地址：**<https://spaces.ac.cn/archives/7500>_
...
year={2020},  
month={Jun},  
url={\url{https://spaces.ac.cn/archives/7500}},  
} 
