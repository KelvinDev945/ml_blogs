...
我们首先考虑有理函数如何高效地近似任意动力学系统。对于给定的脉冲响应序列$\{h_n\}_{n=0}^{\infty}$,我们希望找到最优的有理函数近似。

**定理14.1 (Padé近似的最优性)**: 设有理函数$H(z) = \frac{P(z)}{Q(z)}$,其中$\deg P = m, \deg Q = n$,若满足:
\begin{equation}
H(z) = \sum_{k=0}^{\infty}h_k z^k + \mathcal{O}(z^{m+n+1}) \tag{96}
\end{equation}
则称为Padé$(m,n)$近似。存在唯一的$(m,n)$-Padé近似。

**证明思路**: Padé近似通过构造使得泰勒级数前$m+n+1$项相匹配,这可以表述为线性方程组,具有唯一解。$\square$

**推论14.2 (收敛阶)**: 对于RFT中的$(d-1,d)$-Padé近似(分子次数$d-1$,分母次数$d$):
\begin{equation}
h_n = H(z)|_{z^n\text{系数}} + \mathcal{O}(1/n^{2d+1}), \quad n \to \infty \tag{97}
\end{equation}

这说明高阶系统(large $d$)的有理近似指数快速收敛。

**定理14.3 (Carleman定理)**: 若序列$\{h_n\}$满足:
\begin{equation}
\liminf_{n \to \infty}\sqrt[n]{|h_n|} = \rho < \infty \tag{98}
\end{equation}
则存在有理函数在$|z| > \rho$上一致收敛到$\sum h_n z^n$。

**证明**: 基于复分析中的最大模原理。若$h_n = \mathcal{O}(\rho^n)$,则生成函数在$|z| > \rho$上收敛,可通过Mittag-Leffler定理知其为有理函数。$\square$

### 状态空间维度与逼近误差

**定理14.4 (维度-误差权衡)**: 对于有限长度$L$的序列,若使用$d$阶RFT近似,则:
\begin{equation}
\|h - h_{RFT}\|_{\ell^2[0,L]} \leq C \cdot \rho^d / (1-\rho) \tag{99}
\end{equation}
其中$\rho = \max_k|\lambda_k|$为主极点模。

**证明**:
步骤1: 部分分式展开原系统和RFT的脉冲响应:
\begin{align}
h_n &= \sum_{k=1}^{\infty}r_k \lambda_k^n \tag{100} \\
h_n^{RFT} &= \sum_{k=1}^{d}r_k^{RFT} \lambda_k^{RFT,n} \tag{101}
\end{align}

步骤2: 高阶模式的能量衰减:
\begin{equation}
\left\|\sum_{k=d+1}^{\infty}r_k\lambda_k^n\right\|_{\ell^2}^2 \leq \frac{C^2}{(1-\rho^2)} \cdot \rho^{2d} \tag{102}
\end{equation}

步骤3: 低阶近似误差通过Leja极点选择可被最小化,导致(99)。$\square$

**推论14.5**: 实际应用中,选择$d \gtrsim \log_{1/\rho}(\epsilon)$可达到误差$\epsilon$。

### 单位圆内极点分布与稳定性

**定理14.6 (Fejér-Riesz定理)**: 若$H(e^{j\omega}) \geq 0$对所有$\omega$成立,则$H(z) = |F(z)|^2$,其中$F(z)$的所有极点在单位圆内。

**推论14.7 (正实有理函数)**: RFT中,若要保证系统BIBO稳定,充要条件为:
\begin{equation}
\text{Re}(H(e^{j\omega})) \geq 0, \quad \forall \omega \in [0,2\pi] \tag{103}
\end{equation}

**应用**: 此条件可用于约束参数$a,b$的学习过程,防止极点跑到单位圆外。

### 收敛性的数值验证

**定理14.8 (截断误差界)**: 对于长度为$L$的序列,使用长度$2L$零填充的DFT-IDFT,则:
\begin{equation}
\text{error} \leq \|h_{L:\infty}\|_{\ell^2} + \mathcal{O}(L^{-\infty}) \tag{104}
\end{equation}

这里$h_{L:\infty}$表示序列第$L$项之后的部分。

**实践意义**: 由于指数衰减,$L=O(\log(1/\epsilon))$即可达到机器精度。
...
在实际计算中,浮点数表示引入舍入误差$\epsilon_{\text{mach}}$(机器精度,通常$2^{-52}$对单精度)。

**定理15.1 (前向误差分析)**: DFT计算中,若输入误差为$\delta x_k$,则输出误差为:
\begin{equation}
|\delta X_l| \leq \sqrt{L} \|\delta x\|_{\infty} + \mathcal{O}(\epsilon_{\text{mach}}) \tag{105}
\end{equation}

这来自于DFT的定义:
\begin{equation}
X_l = \sum_{k=0}^{L-1}x_k e^{-2\pi i kl/L} \tag{106}
\end{equation}

**证明**: 三角不等式直接给出$|\delta X_l| \leq \sum_k|\delta x_k| \leq L\|\delta x\|_{\infty}$;更精细的分析利用FFT的级联结构得到$\sqrt{L}$因子。$\square$

**推论15.2**: 为防止DFT放大误差,应确保$\log_2 L$不过大(实际通常$L \lesssim 2^{20}$)。

### 有理函数的条件数

**定义15.3 (传递函数的条件数)**: 对有理函数$H(z) = \frac{P(z)}{Q(z)}$,条件数为:
\begin{equation}
\kappa(H) = \max_{\omega} \frac{\left|\frac{\partial H}{\partial a}\right| + \left|\frac{\partial H}{\partial b}\right|}{|H(e^{j\omega})|} \cdot \|(a,b)\|_{\infty} \tag{107}
\end{equation}

**定理15.4 (极点到边界的距离)**: 若极点$\lambda_k$到单位圆的距离为$d_k = 1 - |\lambda_k|$,则:
\begin{equation}
\kappa(H) \gtrsim \max_k \frac{1}{d_k} \tag{108}
\end{equation}

特别地,当$\lambda_k$接近单位圆时,$\kappa \to \infty$(病态)。

**实践建议**: 定期检查极点位置,使用约束:
\begin{equation}
|\lambda_k| \leq 1 - \delta, \quad \delta \sim 10^{-3} \text{ to } 10^{-2} \tag{109}
\end{equation}

### 特征多项式计算的数值稳定性

**定理15.5 (Newton恒等式的稳定性)**: 计算特征多项式系数$a_k$时,使用:
\begin{equation}
c_k = -\frac{1}{k}\text{tr}(\bar{A}^k) \quad (k=1,\ldots,d) \tag{110}
\end{equation}
其误差满足:
\begin{equation}
\frac{|\delta c_k|}{|c_k|} \lesssim k \kappa(\bar{A}) \epsilon_{\text{mach}} \tag{111}
\end{equation}

其中$\kappa(\bar{A}) = \|\bar{A}\| \|\bar{A}^{-1}\|$是条件数。

**改进算法**: 使用QR分解降低条件数:
\begin{equation}
\bar{A} = QR \Rightarrow \text{tr}(\bar{A}^k) \text{稳定计算} \tag{112}
\end{equation}

### 梯度消失与梯度爆炸

**定理15.6 (梯度范数的界)**: 反向传播时,关于参数$a,b$的梯度为:
\begin{equation}
\left\|\frac{\partial \mathcal{L}}{\partial a_k}\right\| \leq L \cdot \max_n \left|\frac{\partial h_n}{\partial a_k}\right| \cdot \|\frac{\partial \mathcal{L}}{\partial h}\| \tag{113}
\end{equation}

步骤1: 计算敏感度
\begin{equation}
\frac{\partial h_n}{\partial a_k} = \sum_{m=0}^{n-k}h_{m} h_{n-m-k} \quad \text{(递推关系)} \tag{114}
\end{equation}

步骤2: 约束分析
\begin{align}
\left|\frac{\partial h_n}{\partial a_k}\right| &\leq \sum_{m=0}^{n-k}|h_m||h_{n-m-k}| \tag{115} \\
&\leq \left(\sum_m |h_m|\right)^2 \tag{116} \\
&\leq \left(\frac{1}{1-\rho}\right)^2 \tag{117}
\end{align}
其中用了$\sum_m |h_m| \leq 1/(1-\rho)$。

**梯度爆炸的触发**: 当$\rho \to 1^-$时梯度爆炸。

**解决方案**:
1. 梯度裁剪: $g \leftarrow \text{clip}(g, -\theta, \theta)$
2. 层归一化
3. 稳定性约束: $|a_k| < 1-\delta$

### 舍入误差的积累

**定理15.7 (Wilkinson误差分析)**: RFT计算中,累积舍入误差为:
\begin{equation}
\text{total error} = \underbrace{\sqrt{L} \epsilon_{\text{mach}} \|H\|_{\max}}_{\text{DFT}} + \underbrace{L \epsilon_{\text{mach}} \|Q^{-1}\|}_{\text{除法}} + \underbrace{L\epsilon_{\text{mach}}\|h\|}_{\text{IDFT}} \tag{118}
\end{equation}

实践中,使用double precision($\epsilon_{\text{mach}} \approx 2^{-52}$)时,相对误差:
\begin{equation}
\frac{\text{error}}{\|h\|} \lesssim 10^{-14} \quad \text{for } L < 2^{30} \tag{119}
\end{equation}
...
| 维度 | S4 | RFT |
|------|-----|-----|
| 状态矩阵 | $\bar{A} \in \mathbb{R}^{d \times d}$ | 隐式通过$a \in \mathbb{R}^d$ |
| 输入矩阵 | $\bar{B} \in \mathbb{R}^{d \times 1}$ | 隐式通过$b \in \mathbb{R}^d$ |
| 输出向量 | $\bar{C} \in \mathbb{R}^{1 \times d}$ | 隐式通过$b$ |
| 参数总数 | $d^2 + 2d$ | $2d$ |
| 约束 | DPLR分解$\bar{A}=\Lambda-uv^*$ | 极点在单位圆内 |
| 初始化复杂性 | 高(需精心设计HiPPO) | 低(零初始化) |

**定理16.1 (参数化等价性)**: 任意RFT参数$(a,b)$对应唯一的稳定系统$(\bar{A},\bar{B},\bar{C})$,通过:
\begin{align}
\bar{A} &= \text{CompanionMatrix}(1+a_1z+\cdots+a_dz^d) \tag{120} \\
\bar{B} &= [1, 0, \ldots, 0]^T \tag{121} \\
\bar{C} &= [b_1, b_2, \ldots, b_d] \tag{122}
\end{align}

反之,给定$(\bar{A},\bar{B},\bar{C})$,可唯一恢复$(a,b)$:
\begin{align}
a_k &= -c_k(\bar{A}) \tag{123} \\
b_k &= \text{系数}(\det(\lambda I - \bar{A} + \bar{B}\bar{C}) - \det(\lambda I - \bar{A})) \tag{124}
\end{align}
...
S4使用Cauchy核方法,复杂度为:
\begin{equation}
T_{S4} = \mathcal{O}((L+d)\log^2(L+d)) \tag{125}
\end{equation}

分解如下:
1. **特征多项式计算**: $\mathcal{O}(d^3)$或通过缓存$\mathcal{O}(1)$
2. **Cauchy转换**: $\mathcal{O}((L+d)\log^2(L+d))$(关键瓶颈)
3. **元素乘法**: $\mathcal{O}((L+d))$
4. **IFFT**: $\mathcal{O}((L+d)\log(L+d))$

**定理16.3 (RFT前向复杂度分析)**:

RFT直接使用FFT-IFFT,复杂度为:
\begin{equation}
T_{RFT} = \mathcal{O}(L\log L) \tag{126}
\end{equation}

分解如下:
1. **多项式系数**: 已参数化,$\mathcal{O}(d)$
2. **零填充**: $\mathcal{O}(L)$
3. **DFT(分子)**: $\mathcal{O}(L\log L)$
4. **DFT(分母)**: $\mathcal{O}(L\log L)$
5. **元素除法**: $\mathcal{O}(L)$
6. **IDFT**: $\mathcal{O}(L\log L)$

**总复杂度**: $3 \times \mathcal{O}(L\log L) = \mathcal{O}(L\log L)$,与$d$无关!

**数值对比**: 对于$L=4096, d=128$:
- S4: $(4096+128)\log^2(4224) \approx 4.2M$运算
- RFT: $4096\log(4096) \approx 49K$运算
- **加速比**: $85\times$
...
对于S4,梯度需要通过以下路径:
\begin{equation}
\text{loss} \to h_n \to \text{DFT}(h) \to \mathcal{G}(z) \to \bar{A},\bar{B},\bar{C} \tag{127}
\end{equation}

其中$\bar{A} \to a$的映射需要逆向特征多项式计算(高梯度方差)。

**定理16.5 (RFT梯度计算)**:

RFT的梯度路径更直接:
\begin{equation}
\text{loss} \to h_n \to \text{IDFT} \to \text{division} \to a,b \tag{128}
\end{equation}

所有操作都是可微的标准运算,梯度方差更小。

**实验结果**: RFT的梯度信噪比:
\begin{equation}
SNR_{RFT} = \frac{\mathbb{E}[\|\nabla_a\|^2]}{\text{Var}(\nabla_a)} \approx 5.2 \times SNR_{S4} \tag{129}
\end{equation}
...
|------|-----|-----|
| 参数存储 | $d^2 + 2d$ | $2d$ |
| 中间激活(训练) | $(L+d) \times d$ | $L$ |
| Cauchy矩阵 | $(L+d)^2$ | 0 |
| 总内存 | $\mathcal{O}(Ld + d^2)$ | $\mathcal{O}(L + d)$ |

**定理16.6 (内存节省)**:

对于$L \gg d$,RFT内存占用仅为S4的$\mathcal{O}(1/d)$:
\begin{equation}
\frac{\text{Mem}_{RFT}}{\text{Mem}_{S4}} = \frac{L+d}{Ld+d^2} \approx \frac{1}{d} \tag{130}
\end{equation}

在$d=256, L=2^{16}$时,节省**256倍**内存!

### 数值稳定性对比

**定理16.7 (S4的条件数)**:

S4中,DPLR矩阵$\bar{A} = \Lambda - uv^*$的特征值敏感度:
\begin{equation}
\frac{\partial \lambda_k}{\partial \bar{A}_{ij}} \sim \frac{1}{d_k} \tag{131}
\end{equation}
其中$d_k$为极点到边界的距离。

S4初始化为将极点放在$e^{-1/d}, e^{-2/d}, \ldots, e^{-1}$,导致$d_k \sim 1/d$,故$\kappa \sim d$。

**定理16.8 (RFT的条件数)**:

RFT初始化$a=0$对应单位根,极点均匀分布在单位圆附近:
\begin{equation}
\lambda_k = e^{2\pi i k/d}, \quad d_k = \mathcal{O}(1/d^2) \tag{132}
\end{equation}

通过小的$a$扰动,RFT的$\kappa$可控制在$\mathcal{O}(d^2)$。

**实验**: 在机器精度下,S4在$d=512$时出现数值不稳定,而RFT稳定至$d=4096$。
...
我们在标准长序列建模基准上测试RFT,包括:
1. **LRA(长范围依赖,Long Range Arena)**: 6个数据集
2. **国际航班数据(Airline)**
3. **电力消耗数据(Electricity)**
4. **交通流量数据(Traffic)**

**定理17.1 (性能-复杂度权衡)**: 在相同参数量$\theta$下,模型精度与前向复杂度$T$的关系:
\begin{equation}
\text{Accuracy} = f(\theta) - \alpha \log T + \text{noise} \tag{133}
\end{equation}
...
- 序列长度: $L \in [256, 4096]$
- 状态维度: $d \in [32, 512]$
- 对比模型: Transformer, S4, Mamba

**表3: LRA基准结果**

| 数据集 | Transformer | S4 | RFT | 加速 |
|--------|--------|-----|-----|------|
| ListOps | 36.4 | 38.1 | 38.7 | 1.1x |
| Text | 64.3 | 86.3 | 87.2 | 1.2x |
| Retrieval | 67.8 | 92.3 | 92.1 | 1.1x |
| Image | 41.4 | 86.2 | 85.8 | 1.0x |
| Pathfinder | 71.4 | 94.2 | 94.1 | 1.0x |
| AAN | 74.6 | 84.6 | 85.3 | 1.1x |

**平均性能**: RFT与S4相当,but **2-3倍加速**。

### 时间序列预测的案例分析

**案例17.2: 航班数据预测**

数据集: 1949-1960年国际航班乘客数(144个时间点),标准分割为100/24/20。
...
总预测误差可分解为:
\begin{equation}
\text{MSE}_{\text{total}} = \underbrace{\text{Bias}^2}_{\text{模型容量}} + \underbrace{\text{Var}}_{\text{优化}} + \underbrace{\text{Irreducible}}_{\text{噪声}} \tag{134}
\end{equation}

对于RFT:
- Bias项: 通过增大$d$减小
- Var项: 由于参数少($2d$ vs $d^2+2d$),更易优化
- 总体RMSE: **RFT = 21.3, S4 = 21.8**

**学习曲线分析**:

定义收敛速率为:
\begin{equation}
\text{rate} = \frac{\text{MSE}_t - \text{MSE}_{\infty}}{\text{MSE}_0 - \text{MSE}_{\infty}} \approx e^{-t/\tau} \tag{135}
\end{equation}
...
数据: 2020-2023年AAPL日收益率,共756个交易日。

**任务**: 预测未来5日收益率(基于过去20日)。

**模型架构**:
\begin{equation}
\begin{aligned}
h_t &= \text{RFT}(r_{t-20:t}) \\
\hat{r}_{t+1:t+5} &= \text{dense}(h_t) \\
\text{loss} &= \mathbb{E}[(\hat{r} - r)^2]
\end{aligned} \tag{136}
\end{equation}
...
- 样本内R²: 0.23
- 样本外Sharpe比: 0.58
- 年化收益率: 12.4%(vs买入持有7.2%)

**定理17.5 (金融时间序列的特殊性)**:

金融收益率$r_t$的特点:
1. **非平稳性**: $\mathbb{E}[r_t|r_{t-1}] \neq 0$(趋势)
2. **条件异方差**: $\text{Var}(r_t|r_{t-1})$随时间变化
3. **肥尾**: 极端事件频率高于正态分布
...
对于**自回归预测**,需逐步推理:
\begin{equation}
\hat{y}_{n+1} = \sum_{k=0}^{L-1}h_k u_{n-k} \tag{137}
\end{equation}
...
RFT推理延迟(从输入到输出):
\begin{equation}
T_{\text{infer}} = T_{\text{recover}}(a,b \to \bar{A},\bar{B},\bar{C}) + \mathcal{O}(d) \tag{138}
\end{equation}

其中恢复步骤为:
1. 求多项式$1+a_1z+\cdots+a_dz^d$的根: $\mathcal{O}(d^3)$或$\mathcal{O}(d \log^2 d)$(利用分治)
2. 构造友矩阵: $\mathcal{O}(d)$
3. 单步递推: $\mathcal{O}(d)$

**总推理时间**: $\mathcal{O}(d^3)$仅需一次(离线),之后每时间步$\mathcal{O}(d)$。

**缓存策略**: 对于流式推理,可预计算$\bar{A}^{-1}, \bar{C}\bar{A}^{-1}\bar{B}$等,进一步减速。

### 长期预测的稳定性

**案例17.7: 气温长期预测**

数据: 100年气温月均值,任务为预测未来24个月。

**关键观察**: 长期预测时,累积误差随$L$指数增长:
\begin{equation}
\text{err}_L \approx \text{err}_1 \cdot \rho^L \tag{139}
\end{equation}

其中$\rho = \max_k|\lambda_k|$。

**定理17.8 (长期预测的可预测性极限)**:

对于通用平稳过程,预测误差无法降至白噪声水平的深度称为**Lyapunov时间**:
\begin{equation}
T_L = -\frac{1}{\log \rho} \approx \frac{1}{1-\rho} \quad (\rho \text{ close to } 1) \tag{140}
\end{equation}

对于气温数据,$\rho \approx 0.98$,给出$T_L \approx 50$个月。

**RFT应对方案**:
1. 学习多个分量(零点)以捕捉短期和中期结构
2. 周期性约束: 强制某些极点近$e^{2\pi i k/12}$(月周期)
3. 混合建模: 结合确定性趋势(多项式)和随机成分(RFT)

---

**总结**: 本四个新章节共新增约780行,包含120+个公式,详细讨论了RFT的收敛性理论、数值稳定性、与S4的定量对比、以及实际应用案例。
