...
tags: 优化, 梯度, 优化器, 强化学习, 生成模型
status: pending
---

# 殊途同归的策略梯度与零阶优化

**原文链接**: [https://spaces.ac.cn/archives/7737](https://spaces.ac.cn/archives/7737)
...
[![Gradient](/usr/uploads/2020/09/1506417309.png)](/usr/uploads/2020/09/1506417309.png "点击查看原图")
...
> **排序不等式** 对于$a_1 \geq a_2 \geq \dots \geq a_n$以及$b_1 \geq b_2 \geq \dots \geq b_n$，并假设$(c_1, c_2, \dots, c_n)$是$(b_1, b_2, \dots, b_n)$的任一排列，那么 \begin{equation}\sum_{i=1}^n a_i b_i \geq \sum_{i=1}^n a_i c_i \geq \sum_{i=1}^n a_i b_{n+1-i}\end{equation} 也就是说“同序积和 ≥ 乱序积和 ≥ 倒序积和”。
...
一般来说，求梯度$\nabla_{\theta} p_{\theta}(y|x_t)$并不困难，而$\sum_y$才是主要的困难，因为它要对所有候选类别求和，而实际上的候选类别数目可能大到难以承受，相关讨论在之前的[《漫谈重参数：从正态分布到Gumbel Softmax》](/archives/6705)也出现过。因此，比较好的办法是转化为采样估计的形式，即  
...
此外，之前在[《从采样看优化：可导优化与不可导优化的统一视角》](/archives/7521)介绍的统一视角，也可以看作是一种零阶优化方法，它是对常见优化思路的更统一的推广（通过它可以导出梯度下降、牛顿法、零阶梯度等），但原则上来说，它也存在跟零阶梯度一样的方差大等“通病”，这是零阶优化方法很难避免的。
...
_**转载到请包括本文地址：**<https://spaces.ac.cn/archives/7737>_
