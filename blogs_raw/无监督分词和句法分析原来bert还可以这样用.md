...
tags: 无监督, 新词发现, 生成模型, attention, 优化
status: pending
---

# 无监督分词和句法分析！原来BERT还可以这样用

**原文链接**: [https://spaces.ac.cn/archives/7476](https://spaces.ac.cn/archives/7476)
...
本文建议配合如下文章来读：[《【中文分词系列】 2. 基于切分的新词发现》](/archives/3913)、[《最小熵原理（二）：“当机立断”之词库构建》](/archives/5476)、[《最小熵原理（三）：“飞象过河”之句模版和语言结构》](/archives/5577)。这几篇文章主要是介绍了做无监督分词和句法分析的关键思想：**相关矩阵** 。
...
[![基于BERT的“token-token”相关度计算图示（原句为“欧拉是一名数学家”）](/usr/uploads/2020/06/4059500971.png)](/usr/uploads/2020/06/4059500971.png "点击查看原图")
...
> 
> 注2：可能读者会想到直接用BERT里边的Self Attention的注意力矩阵来作为相关性，但其实并不好：一来，BERT有那么多层，每层都有注意力矩阵，你也不知道哪个好；二来，文章[《Google新作Synthesizer：我们还不够了解自注意力》](/archives/7430)告诉我们，注意力矩阵也许并不像我们想象中的那样工作，它里边的值也并不一定是相关性。
...
[![基于BERT的“span-span”相关度计算图示（原句为“欧拉是一名数学家”）](/usr/uploads/2020/06/1477638077.png)](/usr/uploads/2020/06/1477638077.png "点击查看原图")
...
事实上，有了相关矩阵之后，分词是一个很自然的应用。类似[《【中文分词系列】 2. 基于切分的新词发现》](/archives/3913)和[《最小熵原理（二）：“当机立断”之词库构建》](/archives/5476)，我们只需要考虑相邻token的相关性，设定一个阈值，然后把相关度小于这个阈值的两个token切开，大于等于这个阈值的两个token拼接，就构成了一个简单的分词工具了。在实验中，笔者用$\frac{f(x_i, x_{i+1}) + f(x_{i+1}, x_i)}{2}$作为相邻两个token的相关程度度量。
...
同[《ON-LSTM：用有序神经元表达层次结构》](/archives/6621)这篇论文一样，无监督句法分析的基本思路就是递归地将$\boldsymbol{x}=[x_1,x_2,\dots,x_T]$划分为$((\boldsymbol{x}_{ < k}),(x_k, (\boldsymbol{x}_{ > k})))$三部分（如果用span为单位，那么就将$x_i$改为$e_i$，步骤一样，不赘述）。这有点像聚类，其中$\boldsymbol{x}_{ < k}$是一类，而$\boldsymbol{x}_{\geq k}$是一类，聚类的思路也很普通，就是希望同类之间的相关性尽可能大，异类之间的相关性尽可能小，所以可以提出如下简单目标：  
...
[![基于相关度矩阵的分块聚类图示](/usr/uploads/2020/06/1169070610.png)](/usr/uploads/2020/06/1169070610.png "点击查看原图")
...
[![基于BERT的无监督句法分析效果演示](/usr/uploads/2020/06/1080117526.png)](/usr/uploads/2020/06/1080117526.png "点击查看原图")
...
_**转载到请包括本文地址：**<https://spaces.ac.cn/archives/7476>_
