...
**原文链接**: [https://spaces.ac.cn/archives/7359](https://spaces.ac.cn/archives/7359)
...
[![类别不平衡](/usr/uploads/2020/04/814329974.jpg)](/usr/uploads/2020/04/814329974.jpg "点击查看原图")
...
这个loss的优化方向是让目标类的得分$s_t$变为$s_1,s_2,\dots,s_t$中的最大值。关于softmax的相关内容，还可以参考[《寻求一个光滑的最大值函数》](/archives/3290)、[《函数光滑化杂谈：不可导函数的可导逼近》](/archives/6620)等文章。

现在我们转到多标签分类问题，即从$n$个候选类别中选$k$个目标类别。这种情况下我们一种朴素的做法是用sigmoid激活，然后变成$n$个二分类问题，用二分类的交叉熵之和作为loss。显然，当$n\gg k$时，这种做法会面临着严重的类别不均衡问题，这时候需要一些平衡策略，比如手动调整正负样本的权重、[focal loss](/archives/4733)等。训练完成之后，还需要根据验证集来进一步确定最优的阈值。
...
\end{aligned}\end{equation}  
看上去很复杂，其实很简单，还是以$K$个类别总得分为基本单位，但是允许且仅允许$0$类重复出现。预测的时候，仍然是输出分数最大的$K$个类，但允许重复输出$0$类，等价的效果是 _以$s_0$为阈值，只输出得分大于$s_0$的类_ 。最后的式子显示$\overline{Z}_K$也可以通过递归来计算，所以实现上是没有困难的。
...
为什么这个loss会有效呢？在文章[《寻求一个光滑的最大值函数》](/archives/3290)、[《函数光滑化杂谈：不可导函数的可导逼近》](/archives/6620)中我们都可以知道，$\text{logsumexp}$实际上就是$\max$的光滑近似，所以我们有：  
...
_**转载到请包括本文地址：**<https://spaces.ac.cn/archives/7359>_
